...
# 时间维度模型 - 使用 MySQL DATE_FORMAT 函数
time_model = json.dumps({
    "queryType": "sql_query",
    "sqlQuery": "SELECT DATE_FORMAT(created_time, '%Y-%m-%d') as date_id, YEAR(created_time) as year_num, MONTH(created_time) as month_num, QUARTER(created_time) as quarter_num FROM sf_mj_t WHERE zf = 0 GROUP BY DATE_FORMAT(created_time, '%Y-%m-%d'), YEAR(created_time), MONTH(created_time), QUARTER(created_time)",
